...
if($tp == 'cte'){
    ...
    if($rs->RecordCount()>0){//Tiene combinaciones registradas
        ...
        $r0 = $query->Consultar($campos1,$tablas1,"CB.ID_FICHA = ".$ficha." AND CB.ID_CAT_COMBINACION NOT IN (7,8,10,12)","");
        if($r0->RecordCount()>0){
            while(!$r0->EOF){
            ...
            $criterios = "ID_FICHA=".$ficha." AND FC.ID_CAT_COMBINACION NOT IN (7,8,10)";
    ...
    if($rs->RecordCount()>0){
        ...
        while(!$rs->EOF){
            $tbl .= "<tr class='filas' id='row".$rs->fields['ID_PIEZA']."'><td>".$rs->fields['DESCRIPCION']."</td><td>".$rs->fields['CANTIDAD']."</td><td>".$rs->fields['COMBINACION']."</td><td><button type='button' name='remove' id='".$rs->fields['ID_PIEZA']."' class='btn btn-danger btn-xs btn_rmv_pzs'>Quitar</button></td></tr>";
            $rs->MoveNext();
        }
        $tbl .= "</tbody></table>";
        echo $tbl;
    ...
    if($rs->RecordCount()>0){
        ...
        while(!$rs->EOF){
            $tbl .= "<tr id='row".$rs->fields['ID_COMBINACION']."'><td>".$rs->fields['COMBINACION']."</td><td>".$rs->fields['DESCRIPCION']."</td><td>".$rs->fields['COMPOSICION']."</td><td><button type='button' name='remove' id='".$rs->fields['ID_COMBINACION']."' class='btn btn-danger btn-xs btn_rmv_otro'>Quitar</button></td></tr>";
            $rs->MoveNext();
        }
        $tbl .= "</tbody></table>";
        echo $tbl;
    ...
    if($rs->RecordCount()>0){//Revisamos si ya tiene ficha registrada
        ...
        if($rm->RecordCount()>0){//Revisamos que tenga medidas registradas
            ...
            if($r1->RecordCount()>0){//creamos la tabla con las medidas registradas en la bd
                ...
                while(!$r1->EOF){
                    ...
                    $tbl.= "<tr><td><b>".chr($chr)."</b></td><td>";
                        ...
                        $tbl.= "<optgroup>";
                        while(!$rsm->EOF){
                            $tbl.= "<option value='".$rsm->fields['ID_CAT_MED']."' ".($r1->fields['ID_CAT_MED']==$rsm->fields['ID_CAT_MED']?"selected":"").">".$rsm->fields['DESCRIPCION']."</option>";
                            $rsm->MoveNext();
                        }
                        $tbl.= "</optgroup>";
                        $tbl.= "</select></td>";
               ...
               $chr = $chr+1;//sumamos 1 al caracter para imprimir consecutivos, despues del for incrementamos 1 mas
                $med = $med +1;
                for($i=$r1->RecordCount(); $i<=14; $i++){//creamos filas restantes despues de haber creado la tabla con las medidas registradas en la bd -> se construye para registrar nuevas medidas
                    $tbl.= "<tr><td><b>".chr($chr)."</b></td><td>";
                    ...
                    $tbl.= "<optgroup>";
                    while(!$rsm->EOF){
                        $tbl.= "<option value='".$rsm->fields['ID_CAT_MED']."'>".$rsm->fields['DESCRIPCION']."</option>";
                        $rsm->MoveNext();
                    }
                    $tbl.= "</optgroup>";
                    $tbl.= "</select>";
            ...
            $chr = 64;
            for($i=1; $i<=15; $i++){//creamos las filas
    ...
    $id = $_POST['ficha'];
    $tbl = "";
    $rm = $query->Consultar("CB.ID_COMBINACION, CB.ID_CAT_COMBINACION, CT.CORTA","FCOMBINACIONES CB INNER JOIN FCAT_COMBINACION CT ON CT.ID_CAT_COMBINACION = CB.ID_CAT_COMBINACION","CB.ID_FICHA =".$id." AND CB.ID_CAT_COMBINACION NOT IN (8,10,12,13)","CB.ID_COMBINACION");
    if($rm->RecordCount()>0){
        ...
        while(!$rm->EOF){
            ...
            if($rv->RecordCount()>0){
                ...
                while(!$rv->EOF){
                    ...
                    $tbl .= "<td><a href='#' onclick=\"Editar_Variante(".$rv->fields['ID_VARIANTE'].", ".$tipo.",".$rv->fields['VARIANTE'].",'".$rv->fields['COLOR']."','".$tela."');\"><i style='color: green' class='fas fa-edit'></i></a>  ".$rv->fields['ICOD']."_".$rv->fields['IDESCR']."@".$rv->fields['COLOR']."@".$rv->fields['PRV_FACT']."</td>";
                    $rv->MoveNext();
                }
